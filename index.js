...
const current = currentDialogSequence[dialogStep];
  ...
  if (currentMap === 'main') {
    ...
    boundariesM3.forEach(boundary => boundary.draw());
}
...
const current = currentDialogSequence[dialogStep];
   ...
   if (selectedOption === current.correct) {
    ...
    if (lives <= 0) {
        ...
        if (dialogStep >= dialogSequence.length) {
            inDialogScene = false;
            background.image = mapImage;
            dialogStep = 0;
            selectedOption = 0;
    ...
    if (rectangularCollision({ rectangle1: player, rectangle2: interactonZone })) {
        ...
        currentMap = 'main';
    } else if (rectangularCollision({ rectangle1: player, rectangle2: interactionZone2 })) {
        inDialogScene = true;
        dialogStep = 0;
        selectedOption = 0;
        awaitingAnswer = false;
        ...
        currentMap = 'main';
    } else if (score >= 1000 && rectangularCollision({ rectangle1: player, rectangle2: specailZone })) {
        foundBody = true;
        foundBodyTimer = 120; 
    }
}
